'use strict';Object.defineProperty(exports,'__esModule',{value:true});exports.Session=exports.Result=undefined;var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if('value'in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor)}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor}}();var _log=require('./log');var log=_interopRequireWildcard(_log);var _util=require('./util');var util=_interopRequireWildcard(_util);function _interopRequireWildcard(obj){if(obj&&obj.__esModule){return obj}else{var newObj={};if(obj!=null){for(var key in obj){if(Object.prototype.hasOwnProperty.call(obj,key))newObj[key]=obj[key]}}newObj.default=obj;return newObj}}function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError('Cannot call a class as a function')}}// IE fallback (http://afuchs.tumblr.com/post/23550124774/date-now-in-ie8)
Date.now=Date.now||function(){return+new Date};// WAMP "Advanced Profile" support in AutobahnJS per role
// ignore now.
var WAMP_FEATURES={publisher:{},subscriber:{},caller:{},callee:{}};// replace when/function/call
//
function pcall(f){for(var _len=arguments.length,args=Array(_len>1?_len-1:0),_key=1;_key<_len;_key++){args[_key-1]=arguments[_key]}return new Promise(function(resolve,reject){try{resolve(f.call.apply(f,[null].concat(args)))}catch(err){reject(err)}})}// generate a WAMP ID
//
function newid(){return Math.floor(Math.random()*9007199254740992)}var Invocation=function Invocation(procedure,progress,caller,caller_authid,caller_authrole){_classCallCheck(this,Invocation);var self=this;self.procedure=procedure;self.progress=progress;self.caller=caller;self.caller_authid=caller_authid;self.caller_authrole=caller_authrole};var Event=function Event(publication,topic,publisher,publisher_authid,publisher_authrole){_classCallCheck(this,Event);var self=this;self.publication=publication;self.topic=topic;self.publisher=publisher;self.publisher_authid=publisher_authid;self.publisher_authrole=publisher_authrole};var Result=exports.Result=function Result(args,kwargs){_classCallCheck(this,Result);var self=this;self.args=args||[];self.kwargs=kwargs||{}};var Error=function Error(error,args,kwargs){_classCallCheck(this,Error);var self=this;self.error=error;self.args=args||[];self.kwargs=kwargs||{}};var Subscription=function(){function Subscription(topic,handler,options,session,id){_classCallCheck(this,Subscription);var self=this;self.topic=topic;self.handler=handler;self.options=options||{};self.session=session;self.id=id;self.active=true;// this will fire when the handler is unsubscribed
...
self.on_unsubscribe=self._on_unsubscribe.promise}else{self.on_unsubscribe=self._on_unsubscribe}}_createClass(Subscription,[{key:'unsubscribe',value:function unsubscribe(){var self=this;return self.session.unsubscribe(self)}}]);return Subscription}();var Registration=function(){function Registration(procedure,endpoint,options,session,id){_classCallCheck(this,Registration);var self=this;self.procedure=procedure;self.endpoint=endpoint;self.options=options||{};self.session=session;self.id=id;self.active=true;// this will fire when the endpoint is unregistered
...
self.on_unregister=self._on_unregister.promise}else{self.on_unregister=self._on_unregister}}_createClass(Registration,[{key:'unregister',value:function unregister(){var self=this;return self.session.unregister(self)}}]);return Registration}();var Publication=function Publication(id){_classCallCheck(this,Publication);var self=this;self.id=id};var MSG_TYPE={HELLO:1,WELCOME:2,ABORT:3,CHALLENGE:4,AUTHENTICATE:5,GOODBYE:6,HEARTBEAT:7,ERROR:8,PUBLISH:16,PUBLISHED:17,SUBSCRIBE:32,SUBSCRIBED:33,UNSUBSCRIBE:34,UNSUBSCRIBED:35,EVENT:36,CALL:48,CANCEL:49,RESULT:50,REGISTER:64,REGISTERED:65,UNREGISTER:66,UNREGISTERED:67,INVOCATION:68,INTERRUPT:69,YIELD:70};var Session=function(){function Session(socket,defer,onchallenge){_classCallCheck(this,Session);var self=this;// the transport connection (WebSocket object)
...
self._onpong_listeners={};self._publish_reqs={};self._subscribe_reqs={};self._unsubscribe_reqs={};self._call_reqs={};self._register_reqs={};self._unregister_reqs={};// subscriptions in place;
...
self._caller_disclose_me=false;self._publisher_disclose_me=false;self._send_wamp=function(msg){log.debug('send wamp:',msg);// forward WAMP message to be sent to WAMP transport
...
//
pcall(reg.endpoint,args,kwargs,cd).then(function(res){// construct YIELD message
// FIXME: Options
//
...
//
self._socket.onpong=function(){for(var i in self._onpong_listeners){self._onpong_listeners[i]()}};self._socket.onmessage=function(msg){var msg_type=msg[0];// WAMP session not yet open
//
...
//
if('performance'in global&&'now'in performance){self._created=performance.now()}else{self._created=Date.now()}}_createClass(Session,[{key:'log',value:function log(){var self=this;if('console'in global){var header=null;if(self._id&&self._created){var now=null;if('performance'in global&&'now'in performance){now=performance.now()-self._created}else{now=Date.now()-self._created}header='WAMP session '+self._id+' on \''+self._realm+'\' at '+Math.round(now*1000)/1000+' ms'}else{header='WAMP session'}if('group'in console){console.group(header);for(var i=0;i<arguments.length;i+=1){console.log(arguments[i])}console.groupEnd()}else{var items=[header+': '];for(var i=0;i<arguments.length;i+=1){items.push(arguments[i])}console.log.apply(console,items)}}}},{key:'join',value:function join(realm,authmethods,authid){util.assert(typeof realm==='string','Session.join: <realm> must be a string');util.assert(!authmethods||Array.isArray(authmethods),'Session.join: <authmethods> must be an array []');util.assert(!authid||typeof authid==='string','Session.join: <authid> must be a string');var self=this;if(self.isOpen){throw'session already open'}self._goodbye_sent=false;self._realm=realm;var details={};details.roles=WAMP_FEATURES;if(authmethods){details.authmethods=authmethods}if(authid){details.authid=authid}var msg=[MSG_TYPE.HELLO,realm,details];self._send_wamp(msg)}},{key:'leave',value:function leave(reason,message){util.assert(!reason||typeof reason==='string','Session.leave: <reason> must be a string');util.assert(!message||typeof message==='string','Session.leave: <message> must be a string');var self=this;if(!self.isOpen){throw'session not open'}if(!reason){reason='wamp.close.normal'}var details={};if(message){details.message=message}var msg=[MSG_TYPE.GOODBYE,details,reason];self._send_wamp(msg);self._goodbye_sent=true}},{key:'ping',value:function ping(){log.debug('send ping');this._socket.send(undefined)}},{key:'addOnpongListener',value:function addOnpongListener(listener){util.assert(typeof listener==='function','listener must be a function');var self=this;self._onpong_listeners[listener]=listener}},{key:'call',value:function call(procedure,args,kwargs,options){util.assert(typeof procedure==='string','Session.call: <procedure> must be a string');util.assert(!args||Array.isArray(args),'Session.call: <args> must be an array []');util.assert(!kwargs||kwargs instanceof Object,'Session.call: <kwargs> must be an object {}');util.assert(!options||options instanceof Object,'Session.call: <options> must be an object {}');var self=this;if(!self.isOpen){throw'session not open'}options=options||{};// only set option if user hasn't set a value and global option is "on"
...
return d.promise}else{return d}}},{key:'publish',value:function publish(topic,args,kwargs,options){util.assert(typeof topic==='string','Session.publish: <topic> must be a string');util.assert(!args||Array.isArray(args),'Session.publish: <args> must be an array []');util.assert(!kwargs||kwargs instanceof Object,'Session.publish: <kwargs> must be an object {}');util.assert(!options||options instanceof Object,'Session.publish: <options> must be an object {}');var self=this;if(!self.isOpen){throw'session not open'}options=options||{};// only set option if user hasn't set a value and global option is "on"
...
//
self._send_wamp(msg);if(d.promise.then){// whenjs has the actual user promise in an attribute
return d.promise}else{return d}}},{key:'register',value:function register(procedure,endpoint,options){util.assert(typeof procedure==='string','Session.register: <procedure> must be a string');util.assert(typeof endpoint==='function','Session.register: <endpoint> must be a function');util.assert(!options||options instanceof Object,'Session.register: <options> must be an object {}');var self=this;if(!self.isOpen){throw'session not open'}// create an remember new REGISTER request
//
...
//
self._send_wamp(msg);if(d.promise.then){// whenjs has the actual user promise in an attribute
return d.promise}else{return d}}},{key:'unsubscribe',value:function unsubscribe(subscription){util.assert(subscription instanceof Subscription,'Session.unsubscribe: <subscription> must be an instance of class autobahn.Subscription');var self=this;if(!self.isOpen){throw'session not open'}if(!subscription.active||!(subscription.id in self._subscriptions)){throw'subscription not active'}var subs=self._subscriptions[subscription.id];var i=subs.indexOf(subscription);if(i===-1){throw'subscription not active'}// remove handler subscription
...
//
self._send_wamp(msg);if(d.promise.then){// whenjs has the actual user promise in an attribute
return d.promise}else{return d}}},{key:'prefix',value:function prefix(_prefix,uri){util.assert(typeof _prefix==='string','Session.prefix: <prefix> must be a string');util.assert(!uri||typeof uri==='string','Session.prefix: <uri> must be a string or falsy');var self=this;if(uri){self._prefixes[_prefix]=uri}else{if(_prefix in self._prefixes){delete self._prefixes[_prefix]}}}},{key:'resolve',value:function resolve(curie){util.assert(typeof curie==='string','Session.resolve: <curie> must be a string');var self=this;// skip if not a CURIE
var i=curie.indexOf(':');if(i>=0){var prefix=curie.substring(0,i);if(prefix in self._prefixes){return self._prefixes[prefix]+'.'+curie.substring(i+1)}else{return curie}}else{return curie}}},{key:'defer',get:function get(){return this._defer}},{key:'id',get:function get(){return this._id}},{key:'realm',get:function get(){return this._realm}},{key:'isOpen',get:function get(){return this.id!==null}},{key:'features',get:function get(){return this._features}},{key:'caller_disclose_me',get:function get(){return this._caller_disclose_me},set:function set(newValue){this._caller_disclose_me=newValue}},{key:'publisher_disclose_me',get:function get(){return this._publisher_disclose_me},set:function set(newValue){this._publisher_disclose_me=newValue}},{key:'subscriptions',get:function get(){var keys=Object.keys(this._subscriptions);var vals=[];for(var i=0;i<keys.length;++i){vals.push(this._subscriptions[keys[i]])}return vals}},{key:'registrations',get:function get(){var keys=Object.keys(this._registrations);var vals=[];for(var i=0;i<keys.length;++i){vals.push(this._registrations[keys[i]])}return vals}}]);return Session}();exports.Session=Session;